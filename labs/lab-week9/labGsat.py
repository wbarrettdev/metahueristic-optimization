...
class GSAT_solver:
    ...
    def selectGSATvar(self):
        """
        TheGSAT algorithm selects the best variable (minimizing the number of unsatisfied clauses), breaking ties uniformly at random
        :return:
        """
        best_delta = None
        candidates = []
        for v in range(1, self.nVars + 1):
            delta = int(self.breakcounts[v]) - int(self.makecounts[v])
